...
/**
 * @brief: 自定义的I2C总线初始化
 * @param OLED_RCC_APB2PERIPH 时钟源相关参数, 常见如RCC_APB2Periph_GPIOA
 * @param OLED_GPIO_GROUP 使用的引脚族, 如GPIOA
 * @param OLED_SCL SCL引脚口, 如GPIO_Pin_9
 * @param OLED_SDA SDA引脚口, 如GPIO_Pin_10
 * @return 
 */
void OLED_Init(uint32_t OLED_RCC_APB2PERIPH, GPIO_TypeDef* OLED_GPIO_GROUP, uint16_t OLED_SCL, uint16_t OLED_SDA)
{
    MyI2C_Init(OLED_RCC_APB2PERIPH, OLED_GPIO_GROUP, OLED_SCL, OLED_SDA);
    Delay_ms(100);
 ...
 * @param  Page 以左上角为原点，向下方向的行坐标，范围：0-7
 */
...
/**
 * @brief 清除部分区域的函数
 * @param X 起始点横坐标, 单位为像素 0-127
 * @param Y 起始点纵坐标, 单位为像素 0-63
 * @param Width 区域宽度
 * @param Height 区域高度
 */
